...
import { AiUnavailableError, getGeminiClient } from "@server/infra/gemini";
    ...
    try {
      ...
      throw new Error(
        `Không kết nối được dịch vụ embedding tại ${config.embeddingServiceUrl}. ` +
          `Khởi động: cd services/embedding && .venv\\Scripts\\python.exe -m uvicorn main:app --port 8000 ` +
