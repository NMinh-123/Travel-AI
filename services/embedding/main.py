...
Chỉ bind 127.0.0.1. Dịch vụ này không có xác thực, nên nó không được phơi ra mạng.
    ...
    cd services/embedding && .venv\\Scripts\\python.exe -m uvicorn main:app --port 8000
"""
...
# Phải khớp cột vector(1024) trong db/schema.prisma. Đổi số này là phải migrate
# cột và đánh chỉ mục lại toàn bộ, nên nó được kiểm tra ở mỗi lần embed thay vì tin.
EMBEDDING_DIM = 1024

app = FastAPI(title="Ha Giang Travel — Embedding Service", version="1.0.0")
