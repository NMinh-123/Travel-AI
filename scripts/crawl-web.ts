...
import { WEB_SOURCES, type WebSource } from "@data/knowledge/web-sources";

/**
 * Thu thập bản thô của các trang trong data/knowledge/web-sources.ts.
 *
 *   npm run db:crawl
 *
 * Script này CỐ TÌNH không nối thẳng vào đường ống ingest. Nó chỉ ghi văn bản đã bóc ra
 * data/raw-web/ để người biên tập đọc và chắt lọc sang data/knowledge/web.ts. Lý do là
...
const OUT_DIR = path.join(process.cwd(), "data", "raw-web");
...
async function crawlOne(source: WebSource, fetchedAt: string): Promise<CrawlOutcome> {
  const base = { id: source.id, url: source.url, topic: source.domain };
  ...
  const header = [
    ...
    `# Nguồn: ${response.url}`,
    `# Truy xuất: ${fetchedAt}`,
    `# Nhóm nội dung: ${source.domain}`,
    `# Mức tin cậy nguồn: ${source.tier}`,
    `# Mục đích thu thập: ${source.purpose}`,
    "#",
...
async function main() {
  ...
  console.log(
    `Bản thô nằm ở ${path.relative(process.cwd(), OUT_DIR)} — đọc rồi chắt lọc sang data/knowledge/web.ts.`,
  );
